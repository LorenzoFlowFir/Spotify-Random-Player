...
export class SongChallengeService {
  ...
  public songChallenge(cover: string[], titre: string[], day: string[]): void {
      ...
      shareBtn.onclick = function () {
        if (navigator.share) {
          navigator
            .share({
              title: 'Voici une superbe playlist Spotify!',
              text: `Jetez un coup d'œil à cette playlist: "${titre[0]}"`,
              url: cover[0],
            })
            .then(() => console.log('Partage réussi'))
            .catch((error) => console.log('Erreur de partage', error));
    ...
    //TODO : Créer dirrectement cette partie dans un composant
    moreBtn.onclick = () => {
        ...
        for (let i = 1; i < 10; i++) {
          ...
          p.appendChild(
            document.createTextNode(
              this.getCreationDateFromTitle(Number(day[i]))
            )
          );
